...
class MediaAdminUiTest extends OsExistingSiteTestBase {
  ...
  public function testMediaUsage(): void {
    ...
    $this->assertEquals("Adler's Ranch", $rows[0]->find('css', 'td.views-field-nothing-4')->getText());
    $this->assertEquals('Horseshoe Overlook Chapter 1', $rows[1]->find('css', 'td.views-field-nothing-4')->getText());
    $this->assertEquals("Witch's Cauldron, Witch's Cauldron, Reverend Swanson's Bible", $rows[2]->find('css', 'td.views-field-nothing-4')->getText());
