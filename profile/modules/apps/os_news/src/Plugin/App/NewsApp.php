 ...
 *   cpImportId = "os_news_import",
 *   cpImportFilePath = "public://importcsv/os_news.csv"
 * )
 */
class NewsApp extends AppPluginBase {
  ...
  public function validateImportSource(array $form, FormStateInterface $formState) {
    ...
    if ($missing = $faqImport->validateHeaders($data)) {
      $headerMessage = $this->t('The following Header/columns are missing:<ul> @Title @Date @Body @Redirect @Image @Files @Created date @Path</ul></br> The structure of your CSV file probably needs to be updated. Please download the template again.', $missing);
