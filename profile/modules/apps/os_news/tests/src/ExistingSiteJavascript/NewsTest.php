...
class NewsTest extends OsExistingSiteJavascriptTestBase {
  ...
  /**
   * Test news list should not contains any media link.
   */
  public function testListPageNotContainsMedia() {
    ...
    // Attaching media to the news page.
    $assert_session->waitForElementVisible('css', '.media-browser-drop-box');
    $node_upload = $this->getCurrentPage()->find(
      'css', '.field--name-field-attached-media #upmedia'
    );
    $node_upload->click();
    ...
    $news_with_media = $this->getNodeByTitle($title);
    $this->assertNotEmpty($news_with_media);

    // Redirect to the list page.
    $this->visitViaVsite('news', $this->group);
    $this->assertSession()->statusCodeEquals(200);
    $assert_session->elementNotExists(
      'css', '.field--name-field-attached-media'
    );
  }
